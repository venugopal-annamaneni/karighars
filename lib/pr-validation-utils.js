    ...
    for (const item of items) {
      ...
      for (const link of itemLinks) {
        ...
        if (newQtyThisPR > available + 0.001) { // Allow tiny float tolerance
          errors.push(
            `Item "${item.name ? item.name : item.purchase_request_item_name}" linked to "${estItemLabel}": ` +
...
export async function getEstimationItemAllocations(estimationId, projectId, excludePRId = null) {
  try {
    ...
    return result.rows.map(row => {
      const totalWeightage = parseFloat(row.confirmed_weightage) + parseFloat(row.draft_weightage);
      const totalQtyAllocated = parseFloat(row.confirmed_qty_allocated) + parseFloat(row.draft_qty_allocated);
      ...
      return {
        ...
        confirmed_weightage: parseFloat(row.confirmed_weightage),
        draft_weightage: parseFloat(row.draft_weightage),
        available_weightage: Math.max(0, 1.0 - totalWeightage),
        // Full unit tracking
        confirmed_allocated: parseFloat(row.confirmed_qty_allocated),
        draft_allocated: parseFloat(row.draft_qty_allocated),
        available_qty: Math.max(0, totalQty - totalQtyAllocated)
      };
    });
