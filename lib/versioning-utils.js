 ...
 * @param {Array} updatedItems - Array of items with updates (must include stable_item_id)
...
export async function createNewPRVersion(prId, updatedItems, userId, changeSummary) {
  try {
    ...
    // 4. Delete from current table
    await query(`
      DELETE FROM purchase_request_items
      WHERE purchase_request_id = $1
    `, [prId]);
    
    // 5. Create map of updated items by stable_item_id
    ...
    // 6. Re-insert all items with new version
    const itemsAffected = [];
    ...
    for (const item of currentItems) {
      ...
      await query(`
        INSERT INTO purchase_request_items (
          ...
          unit_price, subtotal, gst_percentage, gst_amount,
          amount_before_gst, item_total,
          lifecycle_status, is_direct_purchase, active, status,
          created_at, created_by, updated_at, updated_by
      ...
      `, [
        ...
        itemData.active !== false, // Default to true if not specified
        itemData.status,
        itemData.created_at,
        itemData.created_by,
        userId
    ...
    // 7. Version estimation links (if any exist)
    await versionEstimationLinks(prId, newVersion, userId);
    
    // 8. Update PR-level totals
