...
export async function createNewPRVersion(prId, updatedItems, userId, changeSummary) {
  try {
    ...
    // 5️⃣ Pre-fetch stable_estimation_item_ids for all links in batch
    const estimationItemIdsToFetch = [];
    for (const item of updatedItems) {
      const links = item.estimation_links || [];
      for (const link of links) {
        if (!link.stable_estimation_item_id && link.estimation_item_id) {
          estimationItemIdsToFetch.push(link.estimation_item_id);
        }
      }
    }
    
    const stableEstIdMap = new Map();
    if (estimationItemIdsToFetch.length > 0) {
      const estItemsResult = await query(`
        SELECT id, stable_item_id 
        FROM estimation_items 
        WHERE id = ANY($1)
      `, [estimationItemIdsToFetch]);
      
      estItemsResult.rows.forEach(row => {
        stableEstIdMap.set(row.id, row.stable_item_id);
      });
      
      console.log(`Pre-fetched ${stableEstIdMap.size} stable_estimation_item_ids`);
    }
    ...
    // 7️⃣ Bulk insert items with RETURNING to get id mapping
    const insertedItemsResult = await query(`
      ...
      RETURNING stable_item_id, id
    `, itemValues);
    
    // 8️⃣ Build map: stable_item_id → new purchase_request_item.id
    const stableToIdMap = new Map();
    insertedItemsResult.rows.forEach(row => {
      stableToIdMap.set(row.stable_item_id, row.id);
    });
    
    console.log(`Bulk inserted ${insertedItemsResult.rows.length} items`);
    
    // 9️⃣ Prepare links for bulk insert
    const allLinks = [];
    for (const payloadItem of updatedItems) {
      const currentItem = currentItemsMap.get(payloadItem.stable_item_id);
      ...
      // Get the new purchase_request_item_id for this item
      const newPrItemId = stableToIdMap.get(payloadItem.stable_item_id);
      if (!newPrItemId) continue;
      
      // Process each link
      for (const link of linksToInsert) {
        let stableEstimationItemId = link.stable_estimation_item_id;
        
        // Fetch from pre-loaded map if needed
        ...
        if (stableEstimationItemId) {
          allLinks.push({
            ...
            version: newVersion,
            estimation_item_id: link.estimation_item_id || null,
            stable_estimation_item_id: stableEstimationItemId,
            purchase_request_item_id: newPrItemId,  // Use mapped id
    ...
    if (allLinks.length > 0) {
      ...
      for (const link of allLinks) {
        linkPlaceholders.push(`(
          $${linkIdx++}, $${linkIdx++}, $${linkIdx++}, $${linkIdx++},
          $${linkIdx++}, $${linkIdx++}, $${linkIdx++}, $${linkIdx++}
        )`);
      ...
      await query(`
        INSERT INTO purchase_request_estimation_links (
          stable_item_id, version, estimation_item_id, stable_estimation_item_id,
          purchase_request_item_id, linked_qty,
          unit_purchase_request_item_weightage, notes
    ...
    // 1️⃣1️⃣ Recalculate PR-level totals
    await recalculatePRTotals(prId);
    
    // 1️⃣2️⃣ Determine change type
...
/**
 * Move current estimation links to history
 */
async function moveLinksToHistory(prId, userId) {
  try {
    // Check if there are any links to move
    ...
    console.error('Error moving links to history:', error);
    throw error;
  }
}
