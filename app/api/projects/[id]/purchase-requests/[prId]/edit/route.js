...
// DELETE /api/projects/[id]/purchase-requests/[prId]/edit - Delete items (creates new version)
export async function DELETE(request, { params }) {
  ...
  if (session.user.role !== USER_ROLE.ESTIMATOR && session.user.role !== USER_ROLE.ADMIN) {
    return NextResponse.json({ 
      error: 'Only Estimators and Admins can delete Purchase Request items' 
    }, { status: 403 });
  }

  const { id: projectId, prId } = params;
  const { stable_item_ids } = await request.json();
  ...
  try {
    ...
    // 2. Validate that items being deleted are in 'pending' status
    if (!stable_item_ids || stable_item_ids.length === 0) {
      await query('ROLLBACK');
      return NextResponse.json({
    ...
    const statusCheck = await query(`
      ...
      WHERE stable_item_id = ANY($1::uuid[])
      AND purchase_request_id = $2
    `, [stable_item_ids, prId]);

    const nonDeletableItems = statusCheck.rows.filter(
      item => !canEditItem(item.lifecycle_status)
    );
    ...
    if (nonDeletableItems.length > 0) {
      await query('ROLLBACK');
      return NextResponse.json({
        error: `Cannot delete items with lifecycle status other than 'pending'`,
        non_deletable_items: nonDeletableItems.map(i => ({
          stable_item_id: i.stable_item_id,
          lifecycle_status: i.lifecycle_status
        }))
      }, { status: 400 });
    ...
    const newVersion = await createNewPRVersion(
      ...
      deletedItems,
      session.user.id,
      `Deleted ${stable_item_ids.length} item(s)`
    );
    ...
    return NextResponse.json({
      success: true,
      message: `${stable_item_ids.length} item(s) deleted successfully`,
      version: newVersion
    });
